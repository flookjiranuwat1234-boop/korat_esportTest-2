...
<body class="text-slate-800 font-sans min-h-screen flex antialiased">
    ...
    <script>
        ...
        function openGalleryActionMenu(button, event) { if (event) event.stopPropagation(); let menu = button._galleryActionMenu; if (!menu) { menu = button.parentElement.parentElement.querySelector('.gallery-action-menu'); button._galleryActionMenu = menu; } if (!menu) return; document.querySelectorAll('.gallery-action-menu').forEach(item => item.classList.add('hidden')); const opening = menu.classList.contains('hidden'); if (!opening) { button.setAttribute('aria-expanded', 'false'); return; } document.body.appendChild(menu); menu.classList.remove('hidden'); const rect = button.getBoundingClientRect(); const width = menu.offsetWidth || 208; const height = menu.offsetHeight || 180; menu.style.left = `${Math.max(8, Math.min(rect.right - width, window.innerWidth - width - 8))}px`; menu.style.top = `${rect.bottom + height + 8 <= window.innerHeight - 8 ? rect.bottom + 8 : Math.max(8, rect.top - height - 8)}px`; button.setAttribute('aria-expanded', 'true'); }
