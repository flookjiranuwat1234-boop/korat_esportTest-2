...
function displayTournamentCategoryLabel(?string $categoryCode, ?string $categoryLabel = null): string
{
...
function getTournamentCloseSummary(PDO $pdo, int $tournamentId): array
{
    ...
    $categoryStmt = $pdo->prepare('SELECT tournament_category_id, category_code, COALESCE(label, category_code, \'ไม่ระบุประเภท\') AS category_label
...
function isTournamentReadyToClose(PDO $pdo, int $tournamentId): bool
{
    return getTournamentCloseSummary($pdo, $tournamentId)['ready'];
}

function getTournamentStatusInfo(PDO $pdo, array $tournament): array
{
    $status = strtolower(trim((string) ($tournament['status'] ?? '')));
    $tournamentId = (int) ($tournament['tournament_id'] ?? 0);

    if ($status === 'ongoing' && $tournamentId > 0 && isTournamentReadyToClose($pdo, $tournamentId)) {
    ...
    if (!isset($statusMap[$status])) {
        error_log('Unknown tournament status for display: ' . json_encode(['status' => $status, 'tournament_id' => $tournamentId]));
        return [
            'key' => 'unknown',
            'label' => 'ไม่ทราบสถานะ',
            'class' => 'slate',
            'icon' => 'fa-circle-exclamation',
            'description' => 'สถานะไม่ทราบ: ' . htmlspecialchars($status),
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'complete_tournament') {
    if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        $tournamentId = (int) ($_POST['tournament_id'] ?? 0);
        $tournamentStmt = $pdo->prepare('SELECT tournament_id, status FROM tournaments WHERE tournament_id = :tournament_id');
...
if (isset($_GET['ajax_get_registrations'])) {
    ...
    foreach ($registrations as $registration) {
        ...
        $categoryLabel = displayTournamentCategoryLabel($registration['category_code'] ?? null, $registration['category_label'] ?? null);
        ...
        echo '<div class="flex flex-wrap items-center justify-between gap-2"><div><h4 class="font-bold text-slate-900">' . htmlspecialchars($registration['display_name'] ?? '-') . '</h4><p class="text-xs text-slate-500">ประเภท: ' . htmlspecialchars($categoryLabel) . ' | หัวหน้าทีม: ' . htmlspecialchars($registration['captain_name'] ?? '-') . '</p></div>';
...
$requestedFilterStatus = trim($_GET['status'] ?? '');
$allowedFilterStatuses = ['', 'current', 'registration_open', 'registration_closed', 'checkin_open', 'ongoing', 'ready_to_close', 'completed', 'cancelled'];
$filterStatus = in_array($requestedFilterStatus, $allowedFilterStatuses, true) ? $requestedFilterStatus : '';
...
elseif (in_array($filterStatus, ['registration_open', 'registration_closed', 'checkin_open', 'ongoing', 'completed', 'cancelled'], true)) {
    if ($filterStatus === 'checkin_open') {
        $tournamentWhere[] = 't.checkin_open_at IS NOT NULL AND t.checkin_close_at IS NOT NULL AND t.checkin_open_at <= :admin_now AND t.checkin_close_at >= :admin_now';
        $tournamentParams['admin_now'] = (new DateTimeImmutable('now', new DateTimeZone('Asia/Bangkok')))->format('Y-m-d H:i:s');
    } else {
        $tournamentWhere[] = 't.status = :status_filter';
        $tournamentParams['status_filter'] = $filterStatus;
    }
}
...
if ($filterStatus === 'ready_to_close') {
    $tournaments = array_values(array_filter($tournaments, static function (array $tournament) use ($pdo): bool {
        return getTournamentStatusInfo($pdo, $tournament)['key'] === 'ready_to_close';
    }));
}
...
foreach ($tournaments as $summaryTournament) {
    ...
    if ($summaryStatus === 'ongoing') {
        ...
        $summaryTournaments['completed_count']++;
    } elseif ($summaryStatus === 'ready_to_close') {
        $summaryTournaments['ready_to_close_count']++;
    } elseif ($summaryStatus === 'registration_open') {
        $summaryTournaments['registration_open_count']++;
    } elseif ($summaryStatus === 'checkin_open') {
        $summaryTournaments['checkin_open_count']++;
    }
}
...
foreach ($tournaments as &$tournamentRow) {
    $tournamentRow['close_summary'] = getTournamentCloseSummary($pdo, (int) $tournamentRow['tournament_id']);
}
unset($tournamentRow);
...
// ...existing code...
        ...
        function setupTournamentStepper(form, type) {
            ...
            function validateStep() {
                ...
                if (currentStep === 3 && !form.querySelector('input[name="category_codes[]"]:checked')) { alert('กรุณาเลือกประเภทอย่างน้อย 1 ประเภท'); return false; }
                if (currentStep === 3) {
...
// ...existing code...
        ...
        function openCloseTournamentModal(tournamentId) {
            document.querySelectorAll('.admin-action-menu').forEach(menu => menu.classList.add('hidden'));
            const tournament = tournamentsList.find(item => item.tournament_id == tournamentId);
            if (!tournament) return;
        ...
        function closeCloseTournamentModal() {
            document.getElementById('closeTournamentModal').classList.add('hidden');
            document.getElementById('closeTournamentModal').classList.remove('flex');
        }

        function openCategoryModal(tournamentId, tournamentName) {
            document.getElementById('categoryTitle').innerText = 'ประเภท: ' + tournamentName;
...
<body class="text-slate-800 font-sans min-h-screen flex antialiased">
        ...
        <main class="p-8 space-y-8 flex-1">
            ...
            <div class="flex items-center justify-between bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">รุ่นอายุต่ำกว่า 18 ปี แสดงผลแยกชาย/หญิง | รุ่นทั่วไปแสดงผลแบบรวม</p>
                </div>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <?php foreach ($tournaments as $t): 
                                ...
                                foreach ($activeCategories as $activeCategory) {
                                    ...
                                    $categoryLabel = displayTournamentCategoryLabel($categoryCode, $activeCategory['label'] ?? null);
                                ...
                                <td class="p-4 text-center text-xs">
                                    ...
                                    <div class="flex flex-wrap items-center justify-center gap-2">
                                        <?php if (empty($categorySummaryItems)): ?>
                                            <span class="px-2 py-0.5 rounded border border-amber-200 bg-amber-50 text-amber-700 font-bold">ไม่มีประเภท</span>
                                        <?php else: ?>
                                ...
                                <td class="p-4 text-center">
                                    <?php
                                        $statusBadgeClassMap = [
                                            ...
                                            'checkin_open' => 'bg-blue-50 text-blue-700 border-blue-200',
                                            'bracket_generated' => 'bg-sky-50 text-sky-700 border-sky-200',
                                            'ongoing' => 'bg-violet-50 text-violet-700 border-violet-200',
                                            'ready_to_close' => 'bg-blue-50 text-blue-700 border-blue-200',
                                            'completed' => 'bg-green-700 text-white border-green-800',
                                            'cancelled' => 'bg-red-50 text-red-700 border-red-200',
                                            'unknown' => 'bg-slate-100 text-slate-700 border-slate-200',
                                        ];
                                        $statusBadgeClass = $statusBadgeClassMap[$tournamentStatusInfo['key']] ?? 'bg-slate-100 text-slate-700 border-slate-200';
                                    ?>
                                    <span title="<?php echo htmlspecialchars($tournamentStatusInfo['description']); ?>" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-xs font-bold <?php echo $statusBadgeClass; ?>">
                                        <i class="fa-solid <?php echo htmlspecialchars($tournamentStatusInfo['icon']); ?>"></i>
                                        <?php echo htmlspecialchars($tournamentStatusInfo['label']); ?>
                                    </span>
                                </td>
                    ...
                    <div>
                        ...
                        <select name="game_id" id="create_game_id" onchange="autoFillRules(this, 'create_rules')" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-medium">
                            ...
                            <optgroup label="เกมทีม / เกมเดี่ยวจากระบบ">
                                <?php foreach ($formGames as $game): ?><option value="<?php echo (int) $game['game_id']; ?>" data-game-name="<?php echo htmlspecialchars($game['name'], ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars($game['name']); ?><?php echo !empty($game['play_mode']) ? ' · ' . htmlspecialchars($game['play_mode']) : ''; ?></option><?php endforeach; ?>
                            </optgroup>
                        </select>
                    </div>
    ...
    <div id="categoryModal" class="fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-50 hidden items-center justify-center p-4">
        <div class="bg-white rounded-2xl max-w-4xl w-full p-6 sm:p-8 space-y-5 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between border-b border-slate-100 pb-4"><h3 id="categoryTitle" class="text-lg font-bold text-slate-900">ตั้งค่าประเภท</h3><button type="button" onclick="closeCategoryModal()" class="text-slate-400 p-1"><i class="fa-solid fa-xmark"></i></button></div>
    ...
    <div id="closeTournamentModal" class="fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-[60] hidden items-center justify-center p-4">
        <div class="bg-white rounded-2xl max-w-2xl w-full p-6 sm:p-8 space-y-5 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between border-b border-slate-100 pb-4"><h3 id="closeTournamentTitle" class="text-lg font-bold text-slate-900">ตรวจสอบและจบการแข่งขัน</h3><button type="button" onclick="closeCloseTournamentModal()" class="text-slate-400 p-1"><i class="fa-solid fa-xmark"></i></button></div>
